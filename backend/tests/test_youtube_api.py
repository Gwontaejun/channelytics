...
def test_final_analyze_endpoint_returns_dashboard_ready_response() -> None:
    ...
    assert response.json() == {
        "targetType": "video",
        "target": {"id": "video1", "title": "Video", "thumbnail": None, "channelId": "UC1", "channelTitle": "Channel", "publishedAt": "2026-01-01T00:00:00Z", "likeCount": 98},
        "totalComments": 1,
...
def test_final_analyze_endpoint_resolves_channel_without_collecting_comments() -> None:
    app.dependency_overrides[get_youtube_service] = lambda: FakeCommentService()
    app.dependency_overrides[get_gemini_service] = lambda: FakeGeminiService()
    try:
        response = TestClient(app).post("/api/analyze", json={"url": "https://youtube.com/@example"})
    finally:
        app.dependency_overrides.clear()
